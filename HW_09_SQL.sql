use sakila;
SET SQL_SAFE_UPDATES=0; -- got error code 1175 on one update, so setting safe updates from top.
...
ALTER TABLE actor
ADD COLUMN description TINYBLOB; -- Up to 256 characters
...
WHERE first_name = 'GROUCHO' AND last_name = 'WILLIAMS'; -- Used last name to avoid other GROUCHOs
...
-- SELECT first_name,last_name FROM actor WHERE first_name = 'GROUCHO'; -- Checking
...
SELECT s.first_name,s.last_name,SUM(p.amount) AS 'Amount Rung for August 2005'
...
SELECT f.title, COUNT(fa.actor_id) AS 'Number of Actors Listed'
FROM film f INNER JOIN film_actor fa
ON f.film_id = fa.film_id
GROUP BY f.title;

-- 6d. How many copies of the film `Hunchback Impossible` exist in the inventory system?
SELECT COUNT(inventory_id) AS 'No. of copies for Hunchback Impossible'
FROM inventory
WHERE film_id IN (
...
SELECT cy.city, co.country, SUM(p.amount) AS 'Total Revenue'
FROM staff s
	JOIN address a
		ON s.address_id = a.address_id
	JOIN city cy
...
SELECT ca.name AS 'Genre', SUM(pa.amount) AS 'Gross Revenue'
FROM category ca
	JOIN film_category fc
